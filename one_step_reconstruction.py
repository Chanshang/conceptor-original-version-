...
def parse_args():
    ...
    parser.add_argument("--name", type=str, required=False, default="",  # 一张有关情感的图片
        help="Name of this train",
    )
...
class ConceptDataset(Dataset):  # 定义一个概念数据集类，继承自torch.utils.data.Dataset
    # 构造函数
    def __init__(self, data_root, tokenizer, size=512, repeats=0, interpolation="bicubic",
...
class SelfAttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.unsqueeze(1)
        x, _ = self.attention(x, x, x) # 原形状 [B, 1024] 变成 [B, 1, 1024]
        x = x.squeeze(1)
...
class CrossAttentionBlock(nn.Module):
    def __init__(self, embed_dim, num_heads, mlp_dim, batch_size):
        # 1024, 8, 2048
        ...
        self.fc = nn.Linear(77, 1024)
        self.batch_size = batch_size

    def forward(self, vec, prompt_embedding):
        residual = vec
        vec = self.norm1(vec)

        prompt_embedding = self.fc(prompt_embedding.float())  # F.relu ???
        prompt_embedding = prompt_embedding.repeat(5000 // self.batch_size, 1)# 使用 repeat 方法来扩展张量

        # `vec` 提供 K 和 V，`prompt_embedding` 提供 Q
        # `prompt_embedding` 需要是形状 [B, D_CLIP]
        Q = prompt_embedding.unsqueeze(1)  # [B, 1, D_CLIP]
        vec = vec.unsqueeze(1)
        K = V = vec
        vec, _ = self.cross_attention(Q, K, V) # 原形状 [B, 1024] 变成 [B, 1, 1024]
        vec = vec.squeeze(1)
...
class Net(nn.Module):  # MLP网络，输入是 [5000, 1024]
    ...
    def __init__(self, num_tokens, batch_size):
        ...
        # self.SelfAttentionBlock = SelfAttentionBlock(1024, 4, 2048)
        # self.CrossAttentionBlock = CrossAttentionBlock(1024, 4, 2048, batch_size)

        self.fc1 = nn.Linear(1024, 100)
        self.fc2 = nn.Linear(100, 1)

    def forward(self, x):
        #x = self.SelfAttentionBlock(x)
        #x = self.CrossAttentionBlock(x, prompt_embedding)
...
def main():
    ...
    with open(txt_position, 'w') as file:
        ...
        for i in range(0, len(all_words), 10):
            # 从当前位置取5个单词，如果不足5个，则取剩余的所有单词
            group_of_words = all_words[i:i + 10]
            # 将这一组单词连接成一个字符串，每个单词之间用逗号和空格分隔
            line = ', '.join(group_of_words)
            # 写入文件，并在末尾添加换行符
            file.write(line + '\n')
